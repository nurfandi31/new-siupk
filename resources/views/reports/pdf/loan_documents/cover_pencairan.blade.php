...
<title>COVER PENCAIRAN ({{ $group['name'] . ' - Loan ID. ' . $loan['id'] }})</title>
...
<body>
    <header>
        ...
        <div style="margin: 0px; font-size: 24px;">
            {{ strtoupper('Piutang Kelompok ' . $loan['product_code']) }}
        </div>
        <div style="margin: 0px;">
            Nomor SPK : {{ $loan['loan_number'] }}
        </div>
    </header>
    ...
    <main>
        ...
        <div class="bottom">
            <div style="font-weight: bold;">Alokasi Rp. {{ $alokasi }}</div>
            <div style="font-weight: bold;">Tanggal Pencairan {{ $disbursementDate }}</div>

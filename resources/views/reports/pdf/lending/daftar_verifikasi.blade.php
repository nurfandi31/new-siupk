...
@section('content')
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="0" style="font-size: 11px; margin-bottom: 10px;">
        <tr>
            <td align="center">
                <div style="font-size: 18px; font-weight: bold;">{{ strtoupper($stage_title ?? 'DAFTAR VERIFIKASI PINJAMAN') }}</div>
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="0" style="font-size: 8.5px; table-layout: fixed;">
        <thead>
            <tr style="background: rgb(230, 230, 230); font-weight: bold; text-align: center;">
                ...
                <th class="t l b" width="22%">Peminjam / Kelompok</th>
                <th class="t l b" width="15%">Desa</th>
                <th class="t l b" width="12%">Produk</th>
                <th class="t l b" width="6%">Jenis</th>
                <th class="t l b" width="13%">Pokok Pinjaman</th>
                <th class="t l b" width="10%">{{ $stage_date_label ?? 'Tgl Verifikasi' }}</th>
        ...
        <tbody>
            @forelse ($rows as $row)
                <tr>
                    ...
                    <td class="l b">{{ $row['product_code'] }} {{ $row['product_name'] }}</td>
            ...
            @empty
                <tr>
                    <td class="l b r" colspan="9" align="center" style="padding: 24px;">
                        Tidak ada pinjaman dalam proses verifikasi untuk periode ini.
                    </td>
                </tr>
            @endforelse
        </tbody>
        <tfoot>
            <tr style="background: rgb(232,232,232); font-weight: bold;">
                <th class="t l b" colspan="6" align="left">TOTAL VERIFIKASI ({{ $totals['count'] ?? 0 }})</th>
    ...
    <p style="font-size: 8px; color: #4a5568; margin-top: 8px;">
        Keterangan: {{ $stage_date_label ?? 'Tgl Verifikasi' }} dihitung berdasarkan kolom
        <code>verified_at</code> pada tabel loans. Verifier melihat catatan verifikasi
        pada halaman detail pinjaman.
    </p>
@endsection

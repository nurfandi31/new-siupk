            ...
            <tr>
                <td align="center">
                    <div style="font-size: 18px; font-weight: bold;">
                        DAFTAR PERKEMBANGAN PIUTANG (LPP) MINGGUAN RINCIAN INDIVIDU — {{ strtoupper($prod['product_name']) }} ({{ $prod['product_code'] }})
                    </div>
        ...
        <table border="0" width="100%" cellspacing="0" cellpadding="0" style="font-size: 8px; table-layout: fixed;">
            <thead>
                <tr style="background: rgb(230, 230, 230); font-weight: bold; text-align: center;">
                    <th class="t l b" rowspan="2" width="22%">Peminjam / Loan ID</th>
                    <th class="t l b" rowspan="2" width="10%">NIK</th>
            ...
            <tbody>
                @foreach ($prod['villages'] as $v)
                    <tr style="font-weight: bold;">
                        <td class="l b" colspan="15">DESA: {{ strtoupper($v['village_name']) }}</td>
                    </tr>
                    @foreach ($v['loans'] as $loan)
                        <tr>
                            <td class="l b">{{ $loan['member_name'] }} <span style="color: #718096;">(#{{ $loan['loan_id'] }})</span></td>
                            <td class="l b">{{ $loan['nik'] ?? '-' }}</td>
                    ...
                    <tr style="background: rgb(242,242,242); font-weight: bold;">
                        <td class="l b" colspan="3">Subtotal {{ $v['village_name'] }}</td>
            ...
            <tfoot>
                <tr style="background: rgb(232,232,232); font-weight: bold;">
                    <th class="t l b" colspan="3" align="left">TOTAL {{ $prod['product_code'] }}</th>

    ...
    <tr>
        <td align="center">
            <div style="font-size: 18px; font-weight: bold;">
                KOLEKTIBILITAS PINJAMAN — FORMAT OJK (KBP)
            </div>
...
{{-- Tabel utama per golongan kolektibilitas --}}
<table border="0" width="100%" cellspacing="0" cellpadding="0" style="font-size: 9.5px; table-layout: fixed;">
    <thead>
        <tr style="background: rgb(230, 230, 230); font-weight: bold; text-align: center;">
            ...
            <th class="t l b" rowspan="2" width="14%">Hari Keterlambatan</th>
            <th class="t l b" rowspan="2" width="9%">Jumlah Pinjaman</th>
            <th class="t l b" rowspan="2" width="14%">Pokok Outstanding</th>
            <th class="t l b" rowspan="2" width="9%">%</th>
            <th class="t l b" rowspan="2" width="8%">Penyisihan %</th>
            <th class="t l b" rowspan="2" width="11%">Cadangan Wajib</th>
            <th class="t l b r" rowspan="2" width="11%">Cadangan Dibentuk</th>
        </tr>
    </thead>
    <tbody>
        @forelse($buckets as $idx => $b)
            <tr style="background: {{ $idx % 2 === 0 ? 'rgb(255, 255, 255)' : 'rgb(245, 245, 245)' }};">
                ...
                <td class="l b" align="right">{{ $pct((float) ($b['share_pct'] ?? 0)) }}</td>
                <td class="l b" align="right">{{ $pct((float) ($b['rate_pct'] ?? 0)) }}</td>
                <td class="l b" align="right">{{ $fmt((float) ($b['allowance_required'] ?? 0)) }}</td>
                <td class="l b r" align="right">{{ $fmt((float) ($b['allowance_formed'] ?? 0)) }}</td>
            </tr>
        @empty
    ...
    <tfoot>
        <tr style="background: rgb(200, 200, 200); font-weight: bold;">
            ...
            <td class="t l b" align="right">100.00%</td>
            <td class="t l b"></td>
            <td class="t l b" align="right">{{ $fmt((float) ($totals['allowance_required'] ?? 0)) }}</td>
            <td class="t l b r" align="right">{{ $fmt((float) ($totals['allowance_formed'] ?? 0)) }}</td>
        </tr>
    </tfoot>
</table>

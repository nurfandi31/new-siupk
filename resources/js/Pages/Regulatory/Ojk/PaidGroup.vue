...
const pdfUrl = computed(() => {
    ...
    return `/regulatory/ojk/paid-group/pdf?${q.toString()}`;
});
...
<template>
    <Head title="DRPL — Pinjaman Lunas Kelompok (OJK)" />
    <AuthenticatedLayout>
        <div class="mx-auto max-w-7xl space-y-6">
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div>
                    ...
                    <h1 class="mt-1 text-2xl font-bold text-primary">
                        Rincian Pinjaman Lunas Kelompok
                    </h1>
                    <p class="mt-1 text-sm text-on-surface-variant">
                        Pinjaman kelompok yang telah dilunasi (status completed) pada periode
                ...
                <table class="w-full text-left text-xs border-collapse">
                    <thead class="bg-surface-variant/40 text-on-surface font-semibold">
                        <tr class="border-b border-outline-variant/30">
                            ...
                            <th class="p-3 text-right">Bagi Hasil</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-outline-variant/20">
                        <tr v-for="(row, idx) in rows" :key="row.loan_row_id" class="hover:bg-surface-variant/10">
                            ...
                            <td class="p-2.5 font-medium text-on-surface">
                                {{ row.group_name }}
                                <span v-if="row.group_code" class="text-[11px] text-on-surface-variant">({{ row.group_code }})</span>
                            </td>
            ...
            <AppCard v-if="rows.length === 0" class="p-8 text-center">
                <AppEmptyState
                    icon="inbox"
                    title="Tidak ada pinjaman kelompok lunas"
                    description="Belum ada pinjaman kelompok yang divalidasi pelunasan pada periode ini."
                />
            </AppCard>
        </div>
    </AuthenticatedLayout>

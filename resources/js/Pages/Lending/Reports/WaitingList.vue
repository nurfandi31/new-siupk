...
const baseUrl = '/lending/reports/waiting-list';
const pdfUrl = '/lending/reports/waiting-list/pdf';
const excelUrl = '/lending/reports/waiting-list/excel';
...
<template>
    <Head title="Daftar Waiting List" />
    <AuthenticatedLayout>
        <div class="mx-auto max-w-7xl space-y-6">
            <header class="flex flex-wrap items-end justify-between gap-3">
                <div>
                    <p class="text-xs font-bold uppercase tracking-[0.18em] text-on-surface-variant">Pelaporan Pinjaman</p>
                    <h1 class="mt-1 text-2xl font-bold text-primary">Daftar Waiting List Pinjaman</h1>
                    <p class="mt-1 text-sm text-on-surface-variant">
                        Pinjaman yang masuk antrian persetujuan / pencairan.
            ...
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <AppCard>
                    <p class="text-xs font-bold uppercase tracking-wider text-on-surface-variant">Antrian</p>
            ...
            <AppCard class="overflow-x-auto p-0">
                <table class="min-w-full text-sm">
                    ...
                    <tbody>
                        ...
                        >
                            <td class="px-3 py-2 text-center font-semibold text-primary">{{ row.no }}</td>
                            ...
                            <td class="px-3 py-2 text-center">
                                <AppBadge :tone="row.collector === 'Macet (6+ bln)' ? 'error' : (row.collector === 'Diragukan (4-5 bln)' ? 'warning' : 'neutral')">
                                    {{ row.collector }}
                                </AppBadge>
                            </td>
                            <td class="px-3 py-2 text-center">
                    ...
                    <tfoot v-if="rows.length > 0" class="bg-surface-container-low">
                        <tr class="border-t-2 border-outline font-semibold">
                            <td colspan="6" class="px-3 py-2.5">TOTAL WAITING LIST ({{ totals.count }})</td>

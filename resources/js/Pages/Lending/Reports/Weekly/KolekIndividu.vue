...
function apply(): void {
    router.get(
        '/lending/reports/weekly/kolek-individu',
        {
...
const pdfUrl = computed(() => {
    ...
    return `/lending/reports/weekly/kolek-individu/pdf?${q.toString()}`;
});
...
function kolekLabel(k: number): string {
...
<template>
    <Head title="Kolek Mingguan Individu" />
    <AuthenticatedLayout>
        <div class="mx-auto max-w-7xl space-y-6">
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div>
                    ...
                    <h1 class="mt-1 text-2xl font-bold text-primary">Kolektibilitas Mingguan — Individu</h1>
                            ...
                            <template v-for="v in prod.villages" :key="v.village_name">
                                ...
                                <tr v-for="loanRaw in v.loans" :key="(loanRaw as Record<string, unknown>).loan_id as number" class="hover:bg-surface-variant/10">
                                    ...
                                    <td class="p-2.5 font-medium text-on-surface">
                                        ...
                                        <div class="text-[10px] text-on-surface-variant">NIK: {{ asLoanRow(loanRaw).nik || '-' }}</div>
                                    </td>
                                    <td class="p-2.5 text-right">{{ formatMoney(asLoanRow(loanRaw).alokasi) }}</td>
                                    <td class="p-2.5 text-right font-semibold text-primary">{{ formatMoney(asLoanRow(loanRaw).saldo) }}</td>
                                    <td class="p-2.5 text-right border-l border-outline-variant/20" :class="asLoanRow(loanRaw).tunggakan_pokok > 0 ? 'text-error font-medium' : ''">
                                        {{ formatMoney(asLoanRow(loanRaw).tunggakan_pokok) }}
                                    </td>
                                    <td class="p-2.5 text-right" :class="asLoanRow(loanRaw).tunggakan_jasa > 0 ? 'text-error font-medium' : ''">
                                        {{ formatMoney(asLoanRow(loanRaw).tunggakan_jasa) }}
                                    </td>
                                    <td class="p-2.5 text-center border-l border-outline-variant/20 font-medium" :class="asLoanRow(loanRaw).kolek === 1 ? 'text-secondary' : (asLoanRow(loanRaw).kolek === 2 ? 'text-tertiary' : 'text-error')">
                                        {{ kolekLabel(asLoanRow(loanRaw).kolek) }}
                                    </td>
                                </tr>

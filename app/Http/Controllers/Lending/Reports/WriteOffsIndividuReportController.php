...
use App\Domain\Lending\Services\Reports\WriteOffsIndividuReportService;
use App\Models\User;
...
/**
 * Controller: Pinjaman Dihapusbukukan — INDIVIDU.
 * Route prefix: /lending/reports/write-offs-individu
 */
final class WriteOffsIndividuReportController
{
    public function __construct(
        private readonly PermissionChecker $permissions,
        private readonly WriteOffsIndividuReportService $service,
    ...
    public function index(Request $request): InertiaResponse
    {
        ...
        return Inertia::render('Lending/Reports/WriteOffsIndividu', [
    ...
    public function pdf(Request $request): Response|StreamedResponse
    {
        ...
        return $this->pdf->stream(
            'reports.pdf.lending.write_offs_individu',
            $data,
            sprintf('pinjaman-dihapusbukukan-individu-%04d-%02d.pdf', $year, $month),
            'landscape',
        );
    }
    ...
    public function excel(Request $request): StreamedResponse
    {
        ...
        $product = $request->query('product', 'all');
        $data = $this->service->buildReport($year, $month, is_string($product) ? $product : null);
        $data['title'] = 'DAFTAR PINJAMAN DIHAPUSBUKUKAN — INDIVIDU';

        return $this->excel->writeOffs($data, 'pinjaman-dihapusbukukan-individu', 'Peminjam', 'member_name');
    }

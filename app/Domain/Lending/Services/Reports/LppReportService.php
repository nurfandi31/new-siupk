...
final class LppReportService
{
    ...
    public function buildDesa(int $year, int $month, ?string $productCode = null, ?string $borrowerScope = null): array
    {
        ...
        // Filter K/I: gunakan legacy_source sebagai diskriminator.
        // 'group' = kelompok saja, 'member' = individu saja, null = semua.
        ...
        $loans = $loansQuery
            ->selectRaw('l.row_id, l.id, l.loan_number, l.loan_product_row_id, l.disbursed_at, l.principal_amount, l.legacy_source, g.row_id as group_row_id, g.name as group_name, v.row_id as village_row_id, v.name as village_name, mv.row_id as member_village_row_id, mv.name as member_village_name')
        ...
        foreach ($products as $prod) {
            ...
            foreach ($prodLoans as $loan) {
                // Individu: desa dari member. Kelompok: desa dari group.
    ...
    /**
     * LPP rincian per pinjaman INDIVIDU (per anggota peminjam).
     * Selalu memfilter legacy_source = 'member_loan'.
     *
     * @return array{
     *   year: int,
     *   month: int,
    ...
    public function buildIndividu(int $year, int $month, ?string $productCode = null): array
    {
        ...
        // Pinjaman INDIVIDU: loan_borrowers.member_row_id NOT NULL, group_row_id NULL.
        $loansQuery = DB::connection('tenant')
            ->table('loans as l')
            ->leftJoin('loan_borrowers as b', function ($j): void {
                $j->on('b.tenant_id', '=', 'l.tenant_id')

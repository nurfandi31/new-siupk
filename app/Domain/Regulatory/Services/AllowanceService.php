 ...
 *
 * Laporan compliance untuk menunjukkan bahwa lembaga telah memenuhi
 * kewajiban membentuk CKPN (Cadangan Kerugian Penurunan Nilai) sesuai
 * standar OJK:
 *   Lancar                    → 0.5%  dari outstanding
 *   Dalam Perhatian Khusus    → 3%    dari outstanding
 *   Kurang Lancar             → 10%   dari outstanding
 *   Diragukan                 → 50%   dari outstanding
 *   Macet                     → 100%  dari outstanding
 *
 * Setiap baris menunjukkan:
 *   - Golongan & rentang hari
 *   - Jumlah pinjaman
 *   - Outstanding
 *   - % Penyisihan (tarif OJK)
 *   - Cadangan Wajib (% × outstanding)
 *   - Cadangan Dibentuk (saldo akun 1.1.04.* — Cadangan Kerugian Piutang)
 *   - Selisih = Cadangan Dibentuk − Cadangan Wajib (negatif = kurang)
 *
 * Output menggunakan struktur identik dengan CollectibilityOjkService
 * untuk konsistensi UI, tetapi fokus pada compliance gap (selisih).
 */
final readonly class AllowanceService
{
    ...
    public function buildReport(int $year, int $month, string $borrowerScope = 'all'): array
    {
        ...
        foreach (self::OJK_BUCKETS as $b) {
            ...
            $totals['allowance_required'] = round($totals['allowance_required'] + $required, 2);
        ...
        $compliancePct = $totals['allowance_required'] > 0
            ? round($totals['allowance_formed'] / $totals['allowance_required'] * 100, 2)
            : 100.0;

...
/**
 * Laba Rugi OJK — Laporan Laba Rugi untuk pelaporan OJK.
 *
 * Laporan regulasi OJK yang menampilkan laporan laba rugi lengkap dengan
 * format OJK (klasifikasi Pendapatan Operasional, Beban Operasional,
 * Pendapatan Non-Operasional, Beban Non-Operasional, dan Beban Pajak).
 *
 * Format OJK lebih terstruktur dibanding laporan laba rugi standar dengan
 * pewarnaan sub-total A, B, C sesuai standar OJK:
 *   - A. Laba (Rugi) Operasional  = Pendapatan Ops - Beban Ops
 *   - B. Laba (Rugi) Non Operasional = Pendapatan Non Ops - Beban Non Ops
 *   - C. Laba (Rugi) Sebelum Pajak = A + B
 *   - D. Beban Pajak
 *   - E. Laba (Rugi) Bersih = C - D
 *
 * Sumber data: tabel `accounts` (revenue, expense) + journal_lines
 * (AccountBalanceQuery), konsisten dengan IncomeStatementService existing.
 */
final class OJKIncomeStatementService
{
     ...
     *   summary: array<string, array<string, float>>
     * }
     */
    public function build(int $year, ?int $month): array
    {
        ...
        foreach ($level2Parents as $parent) {
            ...
            $groups[] = [
                ...
                'bucket' => $bucket,
                'ojk_letter' => $this->ojkLetter($bucket),
                'children' => $children,
    ...
    private function ojkLetter(string $bucket): string
    {
        return match ($bucket) {
            'revenue_ops', 'expense_ops' => 'A',
            'revenue_non', 'expense_non' => 'B',
            'tax' => 'D',
            default => '-',
        };
    }
